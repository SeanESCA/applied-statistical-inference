...
Based on the NIC for $\mathcal{F}_{1}$ and $\mathcal{F}_{2}$, 

```{r}
# Increase power of x * theta4 ^ term decreases nic
# nll_expr_q2_4 = expr(lgamma(exp(-theta4)) - 
                       # exp(-theta4) * (log(theta4 ^ 2 - x * theta4) - theta4) + 
                       # (1 - exp(-theta4)) * log(y) + 
                       # exp(-theta4) * (theta4 ^ 2 - x * theta4) * y)
nll_expr_q2_4 = expr(lgamma(exp(-theta4)) - 
                       exp(-theta4) * (log(theta4 ^ 1.6 - x * theta4) - theta4) + 
                       (1 - exp(-theta4)) * log(y) + 
                       exp(-theta4) * (theta4 ^ 1.6 - x * theta4) * y)
nll_pack_q2_4 = deriv_pack(nll_expr_q2_4,
                           c("theta4"),
                           theta_init = 0.5)
opt_res_q2_4 = fit_optim(1, nll_pack_q2_4, dataQ2, sd = 2)
