...
```{r mean_confint}
mean_confint = function(bands, mean_expr, namevec, opt_res) {
  # Calculates the 95% confidence interval for the mean using the delta method.
  temp = bands
  mean_pack = deriv_pack(mean_expr, namevec, c("x"))
  S = solve(opt_res$hessian)
  for (i in 1:n_grid) {
    est = mean_pack$fn(opt_res$par, list("x" = x[i]))
    J = mean_pack$gr(opt_res$par, list("x" = x[i]))
    se = sqrt(J %*% S %*% J)
...
```{r P22_bands_1} 
P22_bands_1 = mean_confint(P22_bands_1, 
                           expr(1/(theta1 - x * theta1 ^ 2)), 
                           c("theta1"), 
                           opt_res_q2_1)
plot_bands(P22_bands_1, dataQ2, main = "")
```

```{r P22_bands_3} 
P22_bands_3 = mean_confint(P22_bands_3, 
                           expr(1/(theta3 - x * theta3 ^ 2)), 
                           c("theta3", "delta3"), 
                           opt_res_q2_3)
plot_bands(P22_bands_3, dataQ2, main = "")
```
...
plot_bands(P23_bands_full, dataQ2, main = "Plot of median for the full model")
```
